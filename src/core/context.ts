...
export class CommandContext {
	...
	constructor(client: Popkat, interaction: ChatInputCommandInteraction) {
		this.client = client;
	...
	public async defer(options?: InteractionDeferReplyOptions) {
		if (this._replyState != ContextReplyStatus.Pending) {
			throw new Error(
				"Cannot defer to an interaction that has already been responded to"
			);
		}
...
export class AutocompleteContext {
	...
	constructor(client: Popkat, interaction: AutocompleteInteraction) {
		this.client = client;
		this.interaction = interaction;
	}
}
