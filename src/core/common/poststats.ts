...
export function validateAction(action: BotListAction) {
	if (!action.enabled)
		throw new Error(
			"<action>.enabled is required in bot_lists in config.yaml"
		);
	if (!action.method)
		throw new Error(
			"<action>.method is required in bot_lists in config.yaml"
		);
	if (!action.interval)
		throw new Error(
			"<action>.interval is required in bot_lists in config.yaml"
		);
	if (!action.url_format)
		throw new Error(
			"<action>.url_format is required in bot_lists in config.yaml"
		);
}
...
function createData(
	...
	if (mod != "u") {
		throw new Error(
			"Only u# is supported for url_format in bot_lists in config.yaml"
		);
	}
	...
	if (authMod != "u" && authMod != "h" && authMod != "b") {
		throw new Error(
			"Only u#, h#, and b# are supported for auth_format in bot_lists in config.yaml"
		);
	}
