...
export class Popkat extends Client {
	...
	async prepare() {
		...
		this.logger.info(
			"Discord",
			`Loaded ${this.teamOwners.length} team owners`,
			this.teamOwners
		);
	...
	private async onReady() {
		this.user.setActivity("Popkat CDN", {
	 ...
	 */
	private async handleBotStaffPerms(
			...
			if (!ctx.interaction.memberPermissions.has(command.userPerms)) {
				if (ctx instanceof CommandContext) {
					try {
						...
						this.logger.error(
							`Command (${command.interactionData.name})`,
	...
	private async onInteraction(interaction: Interaction) {
		...
		if (interaction.isChatInputCommand()) {
			...
			let bsp = await this.handleBotStaffPerms(
				ctx,
				command.botStaffPerms
			);
			if (!bsp) {
				return;
			}
	...
	private async loadCommands() {
		...
		// Commands
		const commandFiles = readdirSync("build/commands").filter((file) =>
			file.endsWith(".js")
		);
		...
		for (const file of commandFiles) {
			this.logger.info(
				"Loader",
				`Loading command ${file.replace(".js", "")}`
			);
			...
			for (let prop of neededProps) {
				if (!command[prop]) {
					throw new Error(
						`Command ${file} is missing property ${prop}`
					);
				}
			}
	...
	private loadEventListeners() {
		if (this.hasLoadedListeners) {
			this.logger.error(
				"Discord",
				"Event listeners have already been loaded"
			);
			return false;
		}
		...
		this.on(Events.ShardError, (error, id) =>
			this.logger.error("Discord", `Shard ${id} error`, error)
		);
		...
		this.on(Events.InteractionCreate, async (interaction) => {
			try {
				await this.onInteraction(interaction).catch((err) => {
					this.logger.error(
						"Discord",
						"Error when handling interaction",
						err
					);
				});
			} catch (err) {
